...
deleteNode.setAttribute("onclick","addCourse(this)")
...
function pick(ctl) {
    ...
    let descriptionPick=$(cols[1]).text();
    let subDatePick=$(cols[2]).text();
    
    let oralMarkPick=$(cols[3]).text();
    let totalMarkPick=$(cols[4]).text();
    ...
    let tNode = document.createTextNode(titlePick);
      let sNode = document.createTextNode(descriptionPick);
      let tyNode =document.createTextNode(subDatePick);
      let stNode =document.createTextNode(oralMarkPick);
      let enNode =document.createTextNode(totalMarkPick);
    ...
    deleteCell.append(deleteNode);
        ...
        deleteNode.classList.add("btn")
        deleteNode.classList.add("btn-primary");
        deleteNode.setAttribute("onclick","editC(this)")
    ...
    function edit(ctl){
      ...
      $(editRow).remove();
          ...
          let tb=document.getElementById("table_courses");
         let tr=tb.getElementsByTagName("tr");
          ...
          for(i=1;i<tr.length;i++){
            ...
            let  td1=tr[i].getElementsByTagName("td")[1];
             ...
             let td5=tr[i].getElementsByTagName("td")[5];
              ...
              let titletemp=td0.textContent;
              let steamtemp=td1.textContent;
              let typetemp=td2.textContent;
              let st_datetemp=td3.textContent;
              let en_datetemp=td4.textContent;
                 
              if(title==titletemp && steam==steamtemp && type==typetemp && st_date==st_datetemp && en_date==en_datetemp){
                 btn= td5.children;
                // btn.style.display = "block";
                 $(btn).show();
              //  x.style.display = "block";
